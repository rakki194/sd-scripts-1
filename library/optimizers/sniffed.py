...
class SNIFFED(Optimizer):
    r"""
    Implements the SNIFFED optimization algorithm: Signed Newton-Iterations For Fast Elegant Descent.

    This optimizer uses sign momentum and two timescales for the denominator (variance/second moment): a short-term and a long-term running average.
    ...
    3. Sign Momentum:
       - Maintains a running average of the sign of the gradients:
         s_t = β_sign * s_{t-1} + (1 - β_sign) * sign(g_t)
       - The update direction is determined by sign(s_t)

    4. Dual Denominator (Variance):
       - Short-term: v_short_t = β2_short * v_short_{t-1} + (1 - β2_short) * g_t²
       - Long-term: v_long_t = β2_long * v_long_{t-1} + (1 - β2_long) * g_t²
       - Denominator: average of both: denom_t = (sqrt(v_short_t) + sqrt(v_long_t)) / 2 + ε
    ...
    θ_t = θ_{t-1} - η_t * sign(s_t) / denom_t

    Where:
    - θ_t: Parameters at step t
    - η_t: Learning rate
    - s_t: Sign momentum (running average of sign(gradient))
    - denom_t: Average of short-term and long-term running std of gradients
    ...
    Arguments:
        ...
        beta_sign (float, optional):
            Coefficient for sign momentum running average (default: 0.9).
        beta2_short (float, optional):
            Coefficient for short-term running average of squared gradients (default: 0.99).
        beta2_long (float, optional):
            Coefficient for long-term running average of squared gradients (default: 0.999).
        eps (float, optional):
    ...
    def __init__(
        ...
        lr: float = 1e-3,
        beta_sign: float = 0.9,
        beta2_short: float = 0.99,
        beta2_long: float = 0.999,
        eps: float = 1e-8,
        ...
        super(SNIFFED, self).__init__(params, defaults)
    ...
    def step(self, closure: Optional[Callable] = None):
        """Perform a single optimization step using sign momentum and dual denominators."""
        loss = None
        ...
        for group in self.param_groups:
            lr = group["lr"]
            beta_sign = group["beta_sign"]
            beta2_short = group["beta2_short"]
            beta2_long = group["beta2_long"]
            eps = group["eps"]
            ...
            for p in group["params"]:
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError("SNIFFED does not support sparse gradients")
                ...
                if len(state) == 0:
                    state["step"] = 0
                    state["sign_momentum"] = torch.zeros_like(p.data)
                    state["ema_squared_short"] = torch.zeros_like(p.data)
                    state["ema_squared_long"] = torch.zeros_like(p.data)
                    state["prev_grad"] = torch.zeros_like(p.data)
                    state["stochastic_applied"] = False

                sign_momentum = state["sign_momentum"]
                ema_squared_short = state["ema_squared_short"]
                ema_squared_long = state["ema_squared_long"]
                ...
                sign_momentum_fp32 = (
                    sign_momentum.to(torch.float32)
                    if sign_momentum.dtype == torch.bfloat16
                    else sign_momentum.clone()
                )
                ema_squared_short_fp32 = (
                    ema_squared_short.to(torch.float32)
                    if ema_squared_short.dtype == torch.bfloat16
                    else ema_squared_short.clone()
                )
                ema_squared_long_fp32 = (
                    ema_squared_long.to(torch.float32)
                    if ema_squared_long.dtype == torch.bfloat16
                    else ema_squared_long.clone()
                )
                prev_grad_fp32 = (
                ...
                # Update short-term and long-term second moments
                ema_squared_short_fp32.mul_(beta2_short).addcmul_(grad_fp32, grad_fp32, value=1 - beta2_short)
                ema_squared_long_fp32.mul_(beta2_long).addcmul_(grad_fp32, grad_fp32, value=1 - beta2_long)

                # Compute denominator (average of both)
                denom = (ema_squared_short_fp32.sqrt() + ema_squared_long_fp32.sqrt()) / 2.0
                denom.add_(eps)

                # Update sign momentum
                sign_momentum_fp32.mul_(beta_sign).add_(grad_fp32.sign(), alpha=1 - beta_sign)

                # Prepare update direction
                update = sign_momentum_fp32.sign().div(denom).mul(lr)
                ...
                if sign_momentum.dtype == torch.bfloat16:
                    if use_stochastic_rounding:
                        (
                            copy_stochastic_(sign_momentum, sign_momentum_fp32)
                            if use_bit_manipulation
                            else sign_momentum.copy_(
                                self.apply_stochastic_bf16_rounding(
                                    sign_momentum_fp32,
                                    probability=stochastic_rounding_prob,
                                    magnitude=stochastic_rounding_magnitude,
                        ...
                        sign_momentum.copy_(sign_momentum_fp32.to(torch.bfloat16))
                else:
                    sign_momentum.copy_(sign_momentum_fp32)

                if ema_squared_short.dtype == torch.bfloat16:
                    if use_stochastic_rounding:
                        (
                            copy_stochastic_(ema_squared_short, ema_squared_short_fp32)
                            if use_bit_manipulation
                            else ema_squared_short.copy_(
                                self.apply_stochastic_bf16_rounding(
                                    ema_squared_short_fp32,
                                    probability=stochastic_rounding_prob,
                                    magnitude=stochastic_rounding_magnitude,
                        ...
                        ema_squared_short.copy_(ema_squared_short_fp32.to(torch.bfloat16))
                else:
                    ema_squared_short.copy_(ema_squared_short_fp32)

                if ema_squared_long.dtype == torch.bfloat16:
                    if use_stochastic_rounding:
                        (
                            copy_stochastic_(ema_squared_long, ema_squared_long_fp32)
                            if use_bit_manipulation
                            else ema_squared_long.copy_(
                                self.apply_stochastic_bf16_rounding(
                                    ema_squared_long_fp32,
                                    probability=stochastic_rounding_prob,
                                    magnitude=stochastic_rounding_magnitude,
                        ...
                        ema_squared_long.copy_(ema_squared_long_fp32.to(torch.bfloat16))
                else:
                    ema_squared_long.copy_(ema_squared_long_fp32)
