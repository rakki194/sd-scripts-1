...
def copy_stochastic_bf16_(target: torch.Tensor, source: torch.Tensor, seed: int = None):
    """
    CUDA version of copy_stochastic_ for bfloat16 target and float32 source, with optional seed.
    """
    if not target.is_cuda or not source.is_cuda:
        raise ValueError("Both tensors must be CUDA tensors")
...
def fused_optimizer(param, ema, ema2, grad, lr, ema_beta, ema2_beta, seed: int = None):
    """
    ...
    Optionally accepts a seed for deterministic behavior.
    """
